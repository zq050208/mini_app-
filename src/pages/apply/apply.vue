  ...
  methods: {
    ...
    async onTapSubmit(e) {
      try {
        ...
        // 如果用的微信授权号码，验证码即签名
        if (this.phoneNumber === this.weixinPhoneNumber) {
          postData.phoneSign = this._data.phoneSign;
        } else {
          postData.validCode = this.validCode;
        }

        await plateServices.addPlate(postData);

        try {
          mpx.requestSubscribeMessage({
            tmplIds: ['1qLcaAG6LoEVBxQygDnoxVLJDaOFOXGKwL4mJqp1DuM'],
          });
        } catch {
          logger.error('订阅授权失败');
        }
        this.$store.dispatch('loadUserCarPlateList');
        router.navigateBack();
      } catch (error) {
        mpx.toast(error.message);
      }
